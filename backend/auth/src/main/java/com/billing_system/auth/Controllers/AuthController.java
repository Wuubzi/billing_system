...
public class AuthController {
  ...
  public ResponseEntity<AuthResponseDTO> verify_code(@Valid @RequestBody VerificationCodeDTO verificationCodeDTO,
      HttpServletRequest request) {
    return new ResponseEntity<>(authService.VerifyCode(verificationCodeDTO, request), HttpStatus.OK);
  }

  @PostMapping("/forgot-password")
  public ResponseEntity<ResponseDTO> reset_password(@Valid @RequestBody ResetPasswordRequestDTO resetPasswordRequestDTO,
      HttpServletRequest request) throws JsonProcessingException {
    return new ResponseEntity<>(authService.ResetPassword(resetPasswordRequestDTO, request), HttpStatus.OK);
  } 

  @PostMapping("/verify-reset-code")
  public ResponseEntity<ResponseDTO> verify_reset_code(
      @Valid @RequestBody ResetPasswordVerifyCodeDTO resetPasswordVerifyCodeDTO, HttpServletRequest request) {
    return new ResponseEntity<>(authService.VerifyResetPasswordCode(resetPasswordVerifyCodeDTO, request),
        HttpStatus.OK);
  }

  @PutMapping("/change-password")
  public ResponseEntity<ResponseDTO> change_password(
